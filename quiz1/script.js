...
function convert(){
    ...
    $.ajax({
        // Get the value of currency in the dropdown menu
        url: `https://api.frankfurter.app/latest?=`+ amount +`&from=USD&to=`+ to,
        method: 'GET',
        dataType: 'json',
        success: function(data) {
            // Call another function to parse and display the weather information
...
function getMoney(data) {
    var to = $('#to-currency').val();
    var new_val = data.rates[to];
    var value = $('#amount').val() * new_val;
    var amount = $('#amount').val();

    var moneyInfo = `<p> ${amount} USD = ${value} `+ to +` </p>`;
    
    $('#money-info').html(moneyInfo);
}
